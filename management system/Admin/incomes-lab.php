...
<body>
   ...
   <h1 class="h1incomesTable"> إيرادات المختبر:-</h1>
<div class="container">
<div class="row">
<div class="col-lg-8">
<?php  echo   msg(); ?> 
...
$sql1 = "SELECT * FROM `employees` WHERE `jop-type`= 'lab doctor' ";
...
?>  
                  ...
                  <option selected>حدد اسم الطبيب :-</option>
          <?php 
$sql = "SELECT * FROM `employees`  WHERE `jop-type`= 'lab technician'";
...
?>  
               ...
               <span class="sum">
                  ...
                  $qty1= 0;
                  $qty2= 0;
                  $sql = "SELECT  `income-value1`,`income-value2` FROM `lab-income` ";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) >0) {
    while($row = mysqli_fetch_assoc($result)) {
        $qty1 += $row['income-value1'];
        $qty2 += $row['income-value2'];
                   
    }
}
                  ?>
            مجموع الإيرادات : <?php $qty=$qty1+$qty2;  echo $qty; ?>
              </span>
    	...
    	?>
                        <tr>
                          ...
                          <td><?php echo htmlentities($row["date"]);  ?></td>
                            <td><a href="DELETE-income-lab.php?income=<?php echo   mysqli_real_escape_string($conn,$row["id"]);   ?>" ><i class="fas fa-trash icon-trash" ></i> </a></td>
                        </tr>
           <?php
    }
}
